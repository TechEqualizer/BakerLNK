...
import CelebrationOverlay from '../components/showcase/CelebrationOverlay';
...
export default function Showcase({ isPublic = false }) {
    ...
    useEffect(() => {
        ...
        // Check if we should show celebration (first visit after onboarding)
        const shouldCelebrate = localStorage.getItem('showCelebration') === 'true';
        if (shouldCelebrate && isPublic) {
    ...
    const HeroSection = () => {
        ...
        return (
            ...
            >
                {hasHeroImage ? (
                    // User uploaded hero image with smart overlay
                    <div 
                    ...
                    >
                        {/* Smart gradient overlay for optimal text readability */}
                        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/70"></div>
                        {/* Additional theme-customizable overlay */}
                        <div 
                            className="absolute inset-0" 
                            style={{ 
                                background: 'var(--theme-hero-overlay, rgba(0,0,0,0.2))'
                            }}
                        ></div>
                    </div>
                ) : (
                ...
                <div className="relative z-10 flex flex-col items-center gap-6 text-white animate-fade-in-up">
                    ...
                    <div className="mt-8 flex flex-col sm:flex-row items-center gap-4">
                        <Button 
                            size="lg" 
                            className="bg-primary text-primary-foreground font-semibold px-8 py-6 rounded-[var(--radius)] transition-all duration-[var(--transition-speed)] hover:scale-[var(--btn-hover-scale)] shadow-[var(--shadow-luxury)]"
                        ...
                        <div className="flex items-center gap-3">
                            {baker.phone_number && (
                                <Button 
                                    ...
                                    onClick={() => window.open(`tel:${baker.phone_number}`, '_self')}
                                >
                                    Call Now
                                </Button>
                            )}
                    ...
                    {/* Social Media Links */}
                    {(baker.instagram_url || baker.facebook_url || baker.tiktok_url || baker.website_url) && (
                        <div className="mt-6 flex items-center justify-center gap-4">
                            {baker.instagram_url && (
                                <Button 
                                    ...
                                    onClick={() => window.open(baker.instagram_url, '_blank')}
                                >
                            ...
                            {baker.facebook_url && (
                                <Button 
                                    ...
                                    size="icon"
                                    className="text-white/80 hover:text-white hover:bg-white/10 rounded-full backdrop-blur-sm"
                                    onClick={() => window.open(baker.facebook_url, '_blank')}
                                >
                                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            {baker.tiktok_url && (
                                <Button 
                                    ...
                                    size="icon"
                                    className="text-white/80 hover:text-white hover:bg-white/10 rounded-full backdrop-blur-sm"
                                    onClick={() => window.open(baker.tiktok_url, '_blank')}
                                >
                                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    return (
        ...
        {showCelebration && (
            <CelebrationOverlay
                bakeryName={baker?.business_name}
